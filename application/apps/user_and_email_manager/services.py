        ...
        if user is None:
            ...
            try:
                with transaction.atomic():
                    ####################################################################### МОжет сюда передавать всё по отдельности что бы можно
                    user = rep.create_user_rep(data, password)
            ####################################################################### было использовать этот реп и в других частях
...
########### Тут воможно сломал было чёт типо CustomUser.objects.filter(dsadsa).update(is_confired=True) Обсудить с Джедаем
...
#### Тут тоже может поломано, было  user = CustomUser.objects.filter(email=email) Может в  репу добавить и обновление
